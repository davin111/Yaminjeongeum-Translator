...
def yaminize(s, degree = 3): # 텍스트 전체와 번역 강도를 인자로 하는 함수
    ...
    outfile = open('output/yamin_results.txt', 'a')
    outfile.writelines('원본 :' + s)
    outfile.writelines('\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n')
    result = ''.join([yaminize_word(word, degree) for word in re.split('([^가-힣ㄱ-ㅎㅏ-ㅣ]+)', s)]) # 한글이 아닌 것을 기준으로 단어를 나눈 후, 그 각각에 대해 함수를 적용하여 변환하고 다시 합쳐 반환
    outfile.writelines('번역본(번역 강도:' + str(degree) + ') :' + result)
    outfile.close()
    return result
